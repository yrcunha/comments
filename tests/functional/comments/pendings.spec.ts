...
test.group('[GET] /posts/{{postId}}/comments/pending', (group) => {
  let user: User
  let post: Post
  ...
  test('invalid parameters for postId', async ({ client }) => {
    const response = await client.get('/posts/A/comments/pending').loginAs(user)
    response.assertUnprocessableEntity()
  })

  test('invalid parameters for pagination', async ({ client }) => {
    const response = await client.get('/posts/1/comments/pending?page=A').loginAs(user)
    response.assertUnprocessableEntity()
  })

  test('success sending pagination', async ({ assert, client }) => {
    const response = await client
      .get(`/posts/${post.id}/comments/pending?page=2&limit=2`)
      .loginAs(user)
    response.assertOk()
  ...
  test('success with default pagination', async ({ assert, client }) => {
    await Comment.create({ postId: post.id, authorId: user.id, text: faker.lorem.paragraph() })
    const response = await client.get(`/posts/${post.id}/comments/pending`).loginAs(user)
    response.assertOk()
